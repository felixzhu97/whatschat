...
| 3 | [components-api-server.puml](./components-api-server.puml) | API server: REST + **GraphQL** (Apollo, feed + nested post, DataLoader, JWT), Posts (**coverUrl** in Cassandra), Notifications (MongoDB, GET/PATCH/POST), WebSocket **notification:new**, Feed, Explore, Comments, Follow, Search, **Ads (accounts/campaigns/groups/creatives, delivery, pacing, spend)**, Admin (recheck, hide, batch), Image/Video/Voice (Media Gen), Vision (tag suggestion, moderation), WebSocket, repos, analytics ingest |
| 3 | [components-web-app.puml](./components-web-app.puml) | Web app: nav, feed, Reels, **notifications sheet** + **search drawer** (left sheets), global search, profile grid (**coverUrl**/coverImageUrl for video), **explore grid** (max-width 963px), create post (**coverUrl** submit, inline violation message), DM, sidebar, i18n, Redux notifications + WS, **sponsored posts rendering + ad click tracking** |
| 3 | [components-mobile-app.puml](./components-mobile-app.puml) | Mobile app: Instagram-style home feed (stories + vertical posts), RTK Query data layer with optimistic like/save/follow, full-screen media/stories viewer, chat/call/settings, analytics events to API |
