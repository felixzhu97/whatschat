...
| 3 | [components-api-server.puml](./components-api-server.puml) | API server: Auth, Users, Messages, Chats, Posts, Feed (followed users with engagement ranking), Comments, Follow, Search, Explore, Recommendations (follow suggestions), WebSocket, repos |
| 3 | [components-web-app.puml](./components-web-app.puml) | Web app: nav, feed (real posts from followed users, multi-photo/video carousel, like/save, engagement counts), Reels, profile (followers/following lists with infinite scroll), explore tab, create post (multi media), DM, right sidebar suggestions, i18n, hooks, API/WS clients |
