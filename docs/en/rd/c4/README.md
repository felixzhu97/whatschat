...
| **2 – Containers** | [containers.puml](./containers.puml) | Web App (:4000), Admin (:4001), Mobile, API Server (:3001), Image Gen (:3457), Video Gen (:3456), PostgreSQL, Redis, Kafka, Local File Storage, Ollama (optional). |
| **3 – Components (API Server)** | [components-api-server.puml](./components-api-server.puml) | NestJS API Server: controllers (incl. AI/Image/Video), WebSocket gateway, services (incl. AI/Image/Video/Analytics), repositories; image prompts refined via Ollama. |
...
- **Containers**: Web App (:4000), Admin (:4001), Mobile, API Server (:3001), Image Gen (:3457), Video Gen (:3456), PostgreSQL, Redis, Kafka, Local File Storage, Ollama (optional). Admin includes Dashboard, Users, Content Safety, Ops Monitor, Business, Data Analytics, System Config, Permission & Audit.
