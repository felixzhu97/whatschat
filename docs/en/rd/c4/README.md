...
| 3 | [components-mobile-app.puml](./components-mobile-app.puml) | Mobile app: Status/Home feed (stories + vertical posts) + Reels tab (vertical video feed), RTK Query data layer with optimistic like/save/follow, cached status/stories/reels, create post flow (pick media -> upload -> create post), full-screen media viewer, chat/call/settings, analytics events to API |
