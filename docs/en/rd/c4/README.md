...
| 3 | [components-api-server.puml](./components-api-server.puml) | API server: REST + **GraphQL** (Apollo, feed + nested post, DataLoader, JWT), media upload `POST /media/upload` (local object storage URL generation), Posts (**coverUrl** in Cassandra), Notifications (MongoDB, GET/PATCH/POST), WebSocket **notification:new**, Feed, Explore, Comments, Follow, Search, **Ads (accounts/campaigns/groups/creatives, delivery, pacing, spend)**, Admin (recheck, hide, batch), Image/Video/Voice (Media Gen), Vision (tag suggestion, moderation), WebSocket, repos, analytics ingest |
