...
| 3 | [components-api-server.puml](./components-api-server.puml) | API：REST + **GraphQL**（Apollo，feed + 嵌套 post，DataLoader，JWT）、认证、用户、消息、聊天、帖子、信息流（互动排序）、探索、评论、关注（推荐）、搜索（帖子/用户/话题、游标、高亮、限流；ES 不可用时用户回退 Prisma）、WebSocket、仓储 |
