...
| 3 | [components-mobile-app.puml](./components-mobile-app.puml) | 移动端：状态/主页信息流（Stories + 纵向帖子）+ Reels 标签（竖屏视频流）、RTK Query 数据层（乐观 like/save/follow、缓存 status/stories/reels）、发帖链路（选择媒体 -> 上传 -> 发帖）、全屏媒体/故事查看、聊天/通话/设置、埋点上报到 API |
