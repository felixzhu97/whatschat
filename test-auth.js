...
async function testUserLogin() {
  ...
  try {
    ...
    if (response.ok && data.success) {
      log("✅ 用户登录成功", "green");
      log(
        `   访问令牌: ${data.data.tokens.accessToken.substring(0, 20)}...`,
        "blue"
      );
      log(
        `   刷新令牌: ${data.data.tokens.refreshToken.substring(0, 20)}...`,
        "blue"
      );
      return {
        success: true,
...
async function testTokenRefresh(refreshToken) {
  ...
  try {
    ...
    if (response.ok && data.success) {
      log("✅ 令牌刷新成功", "green");
      log(
        `   新访问令牌: ${data.data.tokens.accessToken.substring(0, 20)}...`,
        "blue"
      );
      return {
        success: true,
...
async function runTests() {
  ...
  log("\n" + "=".repeat(50), "yellow");
  log(
    `测试完成: ${passedTests}/${tests.length} 项通过`,
