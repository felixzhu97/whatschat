  ...
  private async ensureIndices() {
    ...
    for (const index of indices) {
      ...
      if (!exists) {
        const mappings =
          index === "users"
            ? { properties: { id: { type: "keyword" as const }, username: { type: "text" as const }, createdAt: { type: "date" as const } } }
            : index === "posts"
              ? { properties: { postId: { type: "keyword" as const }, userId: { type: "keyword" as const }, caption: { type: "text" as const }, hashtags: { type: "keyword" as const }, createdAt: { type: "date" as const } } }
