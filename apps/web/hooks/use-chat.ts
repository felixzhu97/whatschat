...
export function useChat(contactId: string) {
  ...
  const sendMessage = useCallback(
    (text: string, type: "text" | "image" | "file" | "voice" = "text", fileData?: any, duration?: number) => {
      ...
      setTimeout(() => {
        setChatState((prev) => ({
          ...prev,
          messages: prev.messages.map((msg) => (msg.id === newMessage.id ? { ...msg, delivered: true } : msg)),
        }))
      }, 1000)
      ...
      setTimeout(() => {
        setChatState((prev) => ({
          ...prev,
          messages: prev.messages.map((msg) => (msg.id === newMessage.id ? { ...msg, read: true } : msg)),
        }))
      }, 3000)
  ...
  const simulateIncomingMessage = useCallback((contactId: string) => {
    const responses = ["好的，我知道了", "没问题！", "让我想想...", "这个想法不错", "我同意你的看法", "稍等，我查一下"]
    ...
    const incomingMessage: Message = {
      ...
      sent: false,
      type: "text",
      sender: contactId.startsWith("group") ? "user" + Math.floor(Math.random() * 3 + 1) : undefined,
      senderName: contactId.startsWith("group") ? ["张三", "李四", "王五"][Math.floor(Math.random() * 3)] : undefined,
    }
