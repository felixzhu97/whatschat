...
export function SearchSuggestions({
  ...
  const longPressEvents = useLongPress({
    onLongPress: (index: number, suggestion: SearchSuggestion) => {
      ...
      if (suggestion.type === "recent" && onRemoveRecentSearch) {
        onRemoveRecentSearch(suggestion.title)
      }
      if ("vibrate" in navigator) {
        navigator.vibrate(50)
      }
    },
    onPress: (suggestion: SearchSuggestion) => onSelectSuggestion(suggestion),
    delay: 500,
  })
  ...
  useEffect(() => {
    ...
    if (!query.trim()) {
      ...
      matchingContacts.forEach((contact) => {
        newSuggestions.push({
          ...
          type: "contact",
          title: contact.name,
          subtitle: contact.isGroup ? `群组 • ${contact.memberCount} 位成员` : "联系人",
      ...
      // 搜索建议
      if (query.length >= 2) {
        const searchSuggestions = [`在所有聊天中搜索 "${query}"`, `在图片中搜索 "${query}"`, `在文件中搜索 "${query}"`]
    ...
    setHoveredIndex(-1)
  }, [query, contacts, recentSearches, isOpen])
      ...
      switch (e.key) {
        ...
        case "Enter":
          ...
          if (hoveredIndex >= 0 && hoveredIndex < suggestions.length) {
            onSelectSuggestion(suggestions[hoveredIndex])
          }
          break
        case "Escape":
          setHoveredIndex(-1)
          ...
          {!query.trim() && recentSearches.length > 0 && (
            <>
              <div className="flex items-center justify-between px-4 py-2">
                ...
                <Button variant="ghost" size="sm" onClick={onClearRecentSearches} className="text-xs text-gray-500 h-6">
                  清除
                </Button>
              </div>
              <Separator />
            ...
            return (
              <div key={suggestion.id}>
                <div
                  ...
                  onClick={() => longPressEvents.onPress(suggestion)}
                  onContextMenu={(e) => {
                    e.preventDefault()
                    if (suggestion.type === "recent" && onRemoveRecentSearch) {
                ...
                >
                  {suggestion.avatar ? (
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback className="text-sm">{suggestion.title[0]}</AvatarFallback>
                    </Avatar>
                  ) : (
                    <div className="w-8 h-8 flex items-center justify-center text-gray-400">{suggestion.icon}</div>
                  )}
                  ...
                  {suggestion.type === "recent" && <div className="text-xs text-gray-400">长按删除</div>}
                </div>
                {index < suggestions.length - 1 && suggestion.type !== suggestions[index + 1]?.type && <Separator />}
              </div>
            )
          })}
        </div>
